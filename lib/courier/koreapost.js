...
var parser = {
  trace: function (body) {
    ...
    $('.table_col').find('tr').each(function (idx) {
      ...
      checkpoint.message.indexOf('Unsuccessful delivery') !== -1 && (isFailAttempt = true)
      checkpoint.message.indexOf('Delivery complete') !== -1 && (isDelivered = true)
    ...
    } else if (isFailAttempt) {
      result.status = tracker.STATUS.FAIL_ATTEMPT
    } else if (result.checkpoints.length > 0) {
      result.status = tracker.STATUS.IN_TRANSIT
    }

    if ([tracker.STATUS.IN_TRANSIT, tracker.STATUS.FAIL_ATTEMPT].indexOf(result.status) !== -1) {
      if ((moment().unix() - moment(result.checkpoints[0].time).unix()) > 259200) {
        result.status = tracker.STATUS.EXCEPTION
      }
    }
