...
var parser = {
  trace: function(body) {
    ...
    var altNumber = $('span[class="tracking-number"]')
      .html()
      .trim()
    var result = {
      courier: courier,
    ...
    var toText = function(txt) {
      ...
      return $(txt.indexOf('<span>') !== -1 ? txt : '<span>' + txt + '</span>')
        .text()
        .trim()
    }
    ...
    // Overall "status" of the shipment is here:
    //  Delivered
    //  Duplicate
    //  Alert
    //   ... etc.
    var textExplanation = $('.text_explanation').html()
    switch (textExplanation) {
      case 'Duplicate':
      case 'Alert':
        result.errorMessage = textExplanation
        break
      default:
        break
    }

    // Tracking History will usually give more details.
    ...
    if (rawTxt) {
      ...
      for (var i = 0; i < rawList.length; i++) {
        ...
        if (list.length > 1 && list[1].trim().length > 0) {
          toText(list[1]).match(/unable to deliver/i) &&
            (status = tracker.STATUS.EXCEPTION)
          toText(list[1]).match(/dead mail/i) &&
            (status = tracker.STATUS.EXCEPTION)
        }
        ...
        var checkpoint = {
          ...
          time: moment(time, 'MMMM DD, YYYY, hh:mm a').format(
            'YYYY-MM-DDTHH:mm'
          ),
        }
      ...
      // Problem getting any useful checkpoint data. So, make one up
      result.errorMessage = textExplanation
      var checkpoint = {
        courier: courier,
        location: 'Unknown',
